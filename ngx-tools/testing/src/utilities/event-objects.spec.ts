import { ENTER } from './../../../keycodes/public-api';
import {
  createMouseEvent,
  createTouchEvent,
  createKeyboardEvent,
...
describe(`event-objects`, () => {
  ...
  describe(`createKeyboardEvent`, () => {
    ...
    test(`should add getters to event`, () => {
      ...
      const actual: KeyboardEvent = createKeyboardEvent('keydown', ENTER, target, 'enter');
      expect(actual.keyCode).toEqual(ENTER);
      expect(actual.key).toEqual('enter');
      expect(actual.target).toEqual(target);
    });


    test(`should add helper for preventDefault`, () => {
      const actual: KeyboardEvent = createKeyboardEvent('keydown', ENTER);
