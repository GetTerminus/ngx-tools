...
describe(`httpRetryer`, () => {
  const caughtError = new HttpErrorResponse({
    status: 503,
  });
  ...
  beforeEach(() => {
    ...
    source = cold('abcd', {a: 1, b: 2, c: 3, d: 4}).pipe(
  ...
  test(`should re-throw HTTP errors not considered (404 for example)`, () => {
    const uncaughtError = new HttpErrorResponse({
      status: 404,
    });
  ...
  test(`retrying connection errors`, () => {
  ...
  describe(`when receiving a 429`, () => {
    test(`should default to backoffStrategy when no header is present`, () => {
      ...
      (expect(
        ...
        cold('ab-ab--abcd', {a: 1, b: 2, c: 3, d: 4}),
      );
    });

    test(`handling a delay with a specific retry ms time`, () => {
      const retryErrorWithMsDelay = new HttpErrorResponse({
        status: 429,
    ...
    test(`handling a delay with a specific retry date time`, () => {
      ...
      (expect(
        ...
        cold('ab' + '-'.repeat(100 - 2) + 'abcd', {a: 1, b: 2, c: 3, d: 4}),
      );
    });
  });

  describe(`when receiving a recoverable error`, () => {
    test(`handling a delay and retry with backoff and reraise if not fixed`, () => {
      (expect(
        ...
        cold('ab-ab--ab#', {a: 1, b: 2}, caughtError),
      );
    });

    test(`retrying a configurable number of times`, () => {
      (expect(
        errorAfter(3, caughtError).pipe(
          httpRetryer({
            retries: 1,
        ...
        cold('ab-ab#', {a: 1, b: 2}, caughtError),
      );
    });
